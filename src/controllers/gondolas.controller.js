...
const queryDB = (rubro, nombre, imagen) => {
  const query = `INSERT INTO [supervisor].[dbo].[gondolasTest] ([rubro],[nombre],[imagen]) 
...
export const POSTGondola = async (req, res) => {
  try {
    ...
    await pool
      .request()
      .query(
        queryDB(req.body["rubro"], req.body["nombre"], req.body["imagen"]),
      );
...
const query = "SELECT  [id] ,[nombre] ,[rubro] ,[imagen] FROM [supervisor].[dbo].[gondolasTest]";
